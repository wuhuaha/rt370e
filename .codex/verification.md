...
Confirm the WS public API/status dump split boundary:
```bash
cd /root/ameba-river
wc -l \
  components/river_cloud/river_xiaozhi_ws_public_api.inc \
  components/river_cloud/river_xiaozhi_ws_status_dump.inc
rg -n "river_xiaozhi_ws_status_dump.inc|river_xiaozhi_dump_status|preview_state|timing_chain_ms|discovery turn_mode" \
  components/river_cloud/river_xiaozhi_ws_public_api.inc \
  components/river_cloud/river_xiaozhi_ws_status_dump.inc
```

Expected result:
- `river_xiaozhi_ws_public_api.inc` is about `719` lines and includes `river_xiaozhi_ws_status_dump.inc` at the bottom
- `river_xiaozhi_dump_status` and the timing/preview/discovery diagnostic logs live in `river_xiaozhi_ws_status_dump.inc`
- public API wrappers keep session/open/send ownership without carrying the large status formatting block inline
...
Expected result:
- status output still includes session, timing age, timing chain, preview state, playback meta, discovery negotiation, and activation diagnostics
- no behavior change is expected; this step only narrows local ownership of the diagnostic formatter
...
Confirm cycle outcome truth is carried from typed result to status diagnostics:
```bash
cd /root/ameba-river
rg -n "downlink_cycle_outcome_t|last_cycle_outcome|downlink_cycle_outcome|outcome=%s|WRITE_RECOVERED|CYCLE_ABORTED" \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c \
  .codex/verification.md
```
...
Expected result:
- the internal runtime truth stores `last_cycle_outcome`
- the cycle result distinguishes inactive, not-ready, acquire miss, write ok,
  write recovered, write failed, aborted, and fallback step policy
- `river xiaozhi status` prints `outcome=<name>` on the downlink line
...
Post-flash board validation for downlink outcome diagnostics:
```text
river xiaozhi status
```

Expected result:
- the downlink line includes `wait=<kind>/<delay>ms outcome=<name> cycle_status=<status>`
- normal playback write cycles can report `outcome=write_ok`
- retry/recovery paths can report `outcome=write_recovered`, `outcome=write_failed`,
  or `outcome=aborted` without inferring that from raw booleans
...
Confirm ready-plan wait reasons are compiled into the tree:
```bash
cd /root/ameba-river
rg -n "WAIT_STARVED|WAIT_SEGMENT_GAP|WAIT_EMPTY|WAIT_PLAYBACK_NOT_READY|cycle_plan\.wait_kind|wait=<kind>" \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c \
  .codex/verification.md
```
...
Expected result:
- the internal wait-kind enum includes the new concrete ready-block reasons
- `prepare_downlink_cycle_plan()` sets `plan.wait_kind` for not-ready branches
- `build_downlink_task_wait_plan()` consumes `cycle_plan.wait_kind`
...
Post-flash board validation for concrete wait reason diagnostics:
```text
river xiaozhi status
```

Expected result:
- the downlink line still includes `wait=.../...ms`
- idle/no-work periods can report `wait=inactive/20ms` or `wait=empty/5ms`
- buffering/gating periods can now report `wait=starved/5ms`, `wait=segment_gap/5ms`,
  or `wait=playback_not_ready/5ms` instead of only generic `not_ready`
...
instead of only a naked worker-step result:
```bash
cd /root/ameba-river
python3 tools/diag/check_codex_harness.py
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'downlink_frame_write_result|write_current_downlink_frame_step|process_downlink_task_cycle' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- write step returns `step_result`, `frame_consumed`, `write_failed` and
  `aborted`
- downlink cycle consumes the typed write result and only projects the worker
  step result at the cycle boundary
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'downlink_retry_frame_pending|keep_current_downlink_frame_for_retry|consume_current_downlink_frame' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'xiaozhi_downlink_runtime_truth\.retry_valid' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- retry-frame pending/keep/consume transitions are centralized in helper
  functions
- remaining direct `retry_valid` touches are confined to those helper functions
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '86,123p' components/river_core/river_dialog_runtime.c
sed -n '228,246p' components/river_core/river_dialog_runtime.c
sed -n '440,452p' components/river_core/river_dialog_runtime.c
sed -n '1192,1202p' components/river_core/river_dialog_runtime.c
rg -n 'derived_facts|error_truth|playback_truth' components/river_core/river_dialog_runtime.c
```
...
- `river_dialog_runtime_derived_facts_t` and `g_river_dialog_runtime.derived_facts`
  no longer appear in `components/river_core/river_dialog_runtime.c`
- `refresh_error_recovering_locked(...)` now updates `error_truth`
- `refresh_playback_locked(...)` now updates `playback_truth`
- `export_playback_error_facts_to_snapshot_locked(...)` now exports from the
  explicit `error_truth` / `playback_truth` state instead of a mixed cache bag
...
`transition_count` back into `derived_facts`:
```bash
cd /root/ameba-river
python3 tools/diag/check_codex_harness.py
git diff --check
...
sed -n '1230,1385p' components/river_core/river_dialog_runtime.c
sed -n '1590,1602p' components/river_core/river_dialog_runtime.c
rg -n 'publish_state|derived_facts\\.interaction_state|derived_facts\\.transition_count|apply_interaction_publish_view_locked' \
  components/river_core/river_dialog_runtime.c
```
...
- `derived_facts.interaction_state` and `derived_facts.transition_count` no
  longer appear in `components/river_core/river_dialog_runtime.c`
- `snapshot.interaction_state`, `snapshot.transition_count`, and
  `snapshot.reason` now all export from the shared `publish_state`
- `publish_locked(...)` now applies:
  - `next_state`
  - `next_transition_count`
  through the typed publish view instead of open-coding publish-state mutation
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '652,666p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1520,1608p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1796,1818p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '2806,2829p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'managed_rebuffer_recovery_result_t|execute_managed_playback_rebuffer_recovery|execute_managed_playback_rebuffer_request|recover_path_io' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- `river_cloud_xiaozhi_execute_managed_playback_rebuffer_recovery(...)` now returns:
  - `river_cloud_xiaozhi_managed_rebuffer_recovery_result_t`
  instead of `river_status_t + recover_path_out`
- `river_cloud_xiaozhi_execute_managed_playback_rebuffer_request(...)` now also returns
  the typed result and uses:
  - `result.status`
  - `result.recover_path`
  for fresh-start fallback logging
- `write_failed` and `upstream starved` callers no longer pass a mutable
  recover-path out parameter into managed execute
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '636,652p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3795,3850p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3890,3910p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'downlink_cycle_plan_t|prepare_downlink_cycle_plan|downlink_cycle_result|process_downlink_task_cycle' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- `river_cloud_xiaozhi_process_downlink_task_cycle(...)` no longer uses:
  - cycle-result branching
  - queued-frames out param
- process cycle now only consumes:
  - `cycle_plan.ready`
  - `cycle_plan.step_result`
  - `cycle_plan.queued_frames`

## Step 5.445
Validate that ready downlink cycle execution now returns the final worker
task-step directly instead of translating through a separate execute-result:
```bash
cd /root/ameba-river
python3 tools/diag/check_codex_harness.py
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '638,652p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3860,3910p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'downlink_task_step_result|execute_ready_downlink_cycle|process_downlink_task_cycle|downlink_execute_result' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- `river_cloud_xiaozhi_execute_ready_downlink_cycle(...)` now returns:
  - `river_cloud_xiaozhi_downlink_task_step_result_t`
- `river_cloud_xiaozhi_process_downlink_task_cycle(...)` no longer translates:
  - execute-result -> task-step
- ready cycle now directly emits:
  - sleep poll on acquire miss / write retry
  - continue on write abort / write progress
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '628,645p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3780,3845p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3985,4020p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'downlink_cycle_result|prepare_downlink_cycle|downlink_active|maybe_rebuffer_starved|maybe_pause_for_segment_gap' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- downlink task no longer inlines:
  ...
  - ack refresh before cycle
  - rebuffer-starved gating
  - segment-gap pause gating
  - empty-queue pending-stop check
  - playback prepare dispatch
- downlink task now only branches on typed cycle result before frame acquire/write
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '628,636p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3785,3810p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3955,3998p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'downlink_frame_acquire_result|acquire_current_downlink_frame|downlink_retry_valid|downlink_task_frame' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- downlink task now only branches on typed frame-acquire result before frame write
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '1590,1695p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3945,3985p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'handle_playback_write_failed|try_write_failed_inline_recover|handle_write_failed_managed_rebuffer' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- playback runtime now defines:
  - `river_cloud_xiaozhi_handle_playback_write_failed(...)`
- downlink task no longer inlines write-failed orchestration; it now delegates to
  the single handler entrypoint and only decides whether to delay/continue
- `write_failed` 的原有 inline recover 与 managed rebuffer fallback 语义保持不变
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '1598,1645p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '2152,2215p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '2348,2405p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '3650,3788p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'set_playback_recovery_path|finish_playback_rebuffer|request_playback_rebuffer_recovery|inline recover replay succeeded|recovery_path=' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- `request_playback_rebuffer_recovery(...)` and the inline `write_failed`
  recover/fallback branches now all update `xiaozhi_playback_recovery_path`
  through the same helper
- `finish_playback_rebuffer()` no longer clears `playback_recovery_path`
- XiaoZhi playback status dump now prints `recovery_path=...`
- inline `service_recover` success can now persist into cloud/dialog snapshot
  observability instead of only rebuffer-managed recovery paths being exported
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '35,170p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '545,645p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1548,1585p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'playback_observe_view|capture_playback_observe_view|playback_observed_phase_kind|playback_phase_known|playback_phase_kind|playback_phase|backend_source\.phase|playback_source\.phase' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- `river_cloud_xiaozhi_playback_observe_view_t` exists and carries phase
  observability
- `river_cloud_xiaozhi_playback_backend_source_t` no longer carries `phase`
- `river_cloud_xiaozhi_fill_playback_runtime_snapshot(...)` reads phase fields
  from `observe_view`
- playback logs/diagnostics use
  `river_cloud_xiaozhi_playback_observed_phase_kind()` instead of
  `playback_source.phase` / `truth_view.backend_source.phase`
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '736,825p' components/river_core/river_dialog_runtime.c
sed -n '930,946p' components/river_core/river_dialog_runtime.c
rg -n 'phase_known|phase_kind|cloud_runtime_available|playback_turn_retains_output_turn_from_projection' \
  components/river_core/river_dialog_runtime.c
```
...
- build output ends with:
  - `Build done`
- `river_dialog_runtime_playback_projection_t` no longer contains:
  - `phase_known`
  - `phase_kind`
- `river_dialog_runtime_capture_playback_projection_locked(...)` no longer copies
  phase fields into projection
- `river_dialog_runtime_playback_turn_retains_output_turn_from_projection(...)`
  uses `!cloud_runtime_available` as the conservative fallback instead of
  `!phase_known`
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '1,80p' components/river_core/river_dialog_runtime.c
sed -n '280,340p' components/river_core/river_dialog_runtime.c
sed -n '616,680p' components/river_core/river_dialog_runtime.c
sed -n '1068,1150p' components/river_core/river_dialog_runtime.c
rg -n 'cloud_playback_facts|export_playback_facts_to_snapshot_locked|capture_playback_projection_locked|import_cloud_snapshot_locked' \
  components/river_core/river_dialog_runtime.c
```
...
- build output ends with:
  - `Build done`
- `river_dialog_runtime_cloud_playback_facts_t` exists
- `capture_playback_projection_locked(...)` now reads playback raw fields from
  `g_river_dialog_runtime.cloud_playback_facts`
- `import_cloud_snapshot_locked(...)` now populates internal playback facts
  first, then mirrors them through
  `river_dialog_runtime_export_playback_facts_to_snapshot_locked(...)`
- exported snapshot remains available for diagnostics/external consumers, but
  playback truth derivation no longer uses it as the internal raw fact source
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '1672,1708p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1790,1826p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'maybe_pause_for_segment_gap|maybe_rebuffer_starved|playback_output_active' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- `river_cloud_xiaozhi_maybe_pause_for_segment_gap()` no longer directly checks
  `g_river_cloud.xiaozhi_playback_active`
- `river_cloud_xiaozhi_maybe_rebuffer_starved()` no longer directly checks
  `g_river_cloud.xiaozhi_playback_active`
- both recovery branches now gate on
  `river_cloud_xiaozhi_playback_output_active()`, so their start/skip behavior
  follows runtime `phase + backend` truth rather than a local shadow bool
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'PLAYBACK_HOLD|playback_hold_kind|playback_segment_gap_hold|hold=%s' \
  include/river/river_cloud.h \
  include/river/river_dialog_runtime.h \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c \
  components/river_core/river_dialog_runtime.c
sed -n '120,150p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1238,1252p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '256,345p' components/river_core/river_dialog_runtime.c
```
...
- build output ends with:
  - `Build done`
- cloud snapshot now exports a typed `playback_hold_kind`
- `owned_paused + waiting_segment` is represented as
  `RIVER_CLOUD_PLAYBACK_HOLD_SEGMENT_GAP`
- dialog runtime no longer derives `playback_recovering` directly from
  `OWNED_PAUSED`
- dialog runtime keeps output continuity via hold truth instead of reusing
  recovery semantics
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'last_fully_heard_(response|playback|segment)_id|playback_last_fully_heard_context_valid|playback ack cleared queued|playback_heard_context' \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1856,1930p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '2198,2248p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1106,1132p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- playback runtime now stores fully-heard response/playback/segment context
- `audio.out.cleared` no longer depends on the current playback-meta
  `response_id / playback_id`
- playback diagnostics now expose the heard-context lineage used by the cleared
  terminal fact
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'playback_quiet_window_allows_vad_open|tts_stop_pending &&|playback_allows_vad_open|capture_held_by_playback' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '327,374p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- playback quiet-window helper now covers both:
  - `waiting_segment`
  - `tts_stop_pending && !playback_output_active`
- capture/VAD gate no longer keeps terminal silent tail-wait in the generic
  playback-held path
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'silent_gap_allows_vad_open|playback_allows_vad_open|capture_held_by_playback|WAITING_SEGMENT' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '248,320p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- `river_cloud_xiaozhi_playback_allows_vad_open()` short-circuits to `true`
  when phase is:
  - `RIVER_CLOUD_PLAYBACK_PHASE_WAITING_SEGMENT`
- `river_cloud_xiaozhi_capture_held_by_playback(...)` returns `false` and clears
  `fallback_reason` when phase is:
  - `RIVER_CLOUD_PLAYBACK_PHASE_WAITING_SEGMENT`
- segment-gap silence no longer keeps capture blocked behind:
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'playback_start_policy_kind|RIVER_CLOUD_PLAYBACK_START_POLICY_' \
  include/river/river_cloud.h \
  include/river/river_dialog_runtime.h \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c \
  components/river_core/river_dialog_runtime.c
rg -n 'playback_start_policy\\[' \
  include/river/river_cloud.h \
  include/river/river_dialog_runtime.h
sed -n '620,706p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '918,930p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '908,916p' components/river_core/river_dialog_runtime.c
```
...
- XiaoZhi playback runtime fills `playback_start_policy_kind` directly from
...
playback-service states as XiaoZhi-owned:
```bash
cd /root/ameba-river
python3 tools/diag/check_codex_harness.py
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
rg -n 'PLAYBACK_BACKEND_|playback_backend_state|playback_backend_owned|playback_backend_needs_start' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'backend=%s|from_backend=%s' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1496,1695p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1848,1898p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- playback runtime exposes typed backend states:
  - `detached`
  - `owned_active`
  - `foreign_active`
  - `restart_pending`
- `playback_check_pending_stop(...)` and `playback_abort_for_cause(...)` use
  backend ownership rather than raw `river_playback_service_active()`
- diagnostics include backend ownership fields in playback logs

## Step 5.274
Validate that local playback-service state now enters core directly through
`dialog runtime`, and that `session_coordinator` no longer owns a playback
listener bridge:
```bash
cd /root/ameba-river
python3 tools/diag/check_codex_harness.py
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '68,78p' include/river/river_dialog_runtime.h
sed -n '508,540p' components/river_core/river_dialog_runtime.c
sed -n '128,136p' components/river_core/river_app.c
rg -n 'on_playback_state\\(' \
  include/river/river_dialog_runtime.h \
  components/river_core/river_dialog_runtime.c \
  components/river_core/river_app.c \
  components/river_core/river_session_coordinator.c \
...
- build output ends with:
  - `Build done`
- `river_app` registers `river_dialog_runtime_on_playback_state(...)`
- `river_session_coordinator_on_playback_state(...)` no longer exists
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '28,95p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '155,175p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '872,910p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '116,132p' components/river_core/river_dialog_runtime.c
rg -n 'playback_phase|refresh_playback_phase|compute_playback_phase' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_core/river_dialog_runtime.c \
  include/river/river_cloud.h \
...
- build output ends with:
  - `Build done`
- XiaoZhi playback runtime owns an explicit `playback_phase` reducer/export path
- cloud/dialog runtime dumps can print cloud playback as:
  - active flag
  - runtime phase
- playback lane occupation is no longer reconstructed from playback-service
  active state inside the XiaoZhi playback runtime helper

## Step 5.261
Validate that XiaoZhi write-failure recovery now waits for a refill threshold
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '640,690p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
sed -n '1650,1695p' components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
rg -n 'rebuffer_resume_ready|playback_rebuffer_pending|playback_write_failed' \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
```
...
- build output ends with:
  - `Build done`
- XiaoZhi downlink worker gates resume on:
  - adaptive rebuffer threshold
- write-failure recovery no longer immediately resumes writes just because the
  playback service stayed active after `flush`

## Step 5.260
Validate that the last generic realtime open reducer left the adapter:
```bash
cd /root/ameba-river
python3 tools/diag/check_codex_harness.py
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '1,120p' components/river_cloud/river_cloud_asr_bridge_runtime.c
sed -n '940,1005p' components/river_cloud/river_cloud_adapter.c
rg -n 'river_cloud_business_time_ready|river_cloud_stream_open_and_flush' \
  components/river_cloud/river_cloud_adapter.c \
...
- ASR bridge runtime owns those helpers and adapter only calls them
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '449,456p' components/river_cloud/river_cloud_internal.h
sed -n '1035,1043p' components/river_cloud/river_cloud_adapter.c
sed -n '1633,1654p' components/river_cloud/river_cloud_xiaozhi_session.c
rg -n 'apply_capture_stream_policy|apply_capture_entry_playback_policy|apply_capture_exit_playback_policy' \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
...
- session runtime exports and implements:
  - `river_cloud_xiaozhi_apply_capture_stream_policy(...)`
- adapter `river_cloud_xiaozhi_stream_push_frame(...)` no longer directly performs:
  - capture-entry playback gate
  - capture-exit playback tail

## Step 5.255
Validate that XiaoZhi stream-push capture/session policy now belongs to
session runtime:
```bash
cd /root/ameba-river
python3 tools/diag/check_codex_harness.py
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '334,340p' components/river_cloud/river_cloud_internal.h
sed -n '446,453p' components/river_cloud/river_cloud_internal.h
sed -n '1040,1062p' components/river_cloud/river_cloud_adapter.c
sed -n '1511,1629p' components/river_cloud/river_cloud_xiaozhi_session.c
rg -n 'apply_stream_push_capture_policy|pre_roll_store\\(|log_stream_open_deferred_once|stream_feed_ok|asr stream active' \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_session.c
```
...
- adapter `river_cloud_xiaozhi_stream_push_frame(...)` no longer directly performs:
  - followup-open `pre_roll` replay loop
  - active-stream `stream_feed_ok/fail` bookkeeping
  - stream activation `asr stream active` transition log
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '447,462p' components/river_cloud/river_cloud_internal.h
sed -n '1640,1665p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '1180,1205p' components/river_cloud/river_cloud_adapter.c
rg -n 'apply_active_stream_capture_policy|cancel_endpoint_soft_close|arm_endpoint_soft_close|complete_active_stream_finish' \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_cloud/river_cloud_adapter.c
```
...
- adapter active-stream push path no longer directly performs:
  - `speech_resumed` endpoint-soft-close cancel
  - `silence_frames` post-roll bookkeeping
  - `duplex_soft_endpoint_enabled()` branching for post-roll close
...
sed -n '193,218p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '125,129p' components/river_cloud/river_cloud_adapter.c
rg -n 'execute_control_transport|execute_open_and_listen_transport|execute_session_control_transport|execute_playback_control_transport|control_execute\\(' \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_cloud/river_cloud_xiaozhi_playback_runtime.c
...
- adapter `river_cloud_xiaozhi_control_execute(...)` now only delegates to the
  runtime-owned control reducer
- the control-op `switch` now lives in runtime rather than adapter
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '351,356p' components/river_cloud/river_cloud_internal.h
sed -n '190,214p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '1685,1716p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '519,540p' components/river_cloud/river_cloud_adapter.c
rg -n 'run_io_tick_housekeeping|run_post_poll_housekeeping|run_post_uplink_housekeeping|finalize_pending_text_if_turn_accepted_after_refresh|check_window_timeout|playback_check_pending_stop' \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_internal.h \
  components/river_cloud/river_cloud_xiaozhi_session.c
```
...
- adapter `river_cloud_xiaozhi_io_task(...)` no longer inlines:
  - `io_tick` refresh / timeout glue
  - post-poll accept finalize glue
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '344,348p' components/river_cloud/river_cloud_internal.h
sed -n '165,281p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '1068,1085p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '430,505p' components/river_cloud/river_cloud_adapter.c
sed -n '1805,1885p' components/river_cloud/river_cloud_adapter.c
rg -n 'trim_uplink_stale_frames|push_pcm\\(|complete_active_stream_finish\\(' \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_cloud/river_cloud_internal.h
```
...
- adapter no longer defines local XiaoZhi helpers for:
  - stale-frame trimming
  - PCM accumulator packetization
  - padded flush on stream finish
- adapter capture/I/O paths now consume runtime-owned uplink ingress / finish
  helpers instead of mutating those runtime facts inline
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '343,353p' components/river_cloud/river_cloud_internal.h
sed -n '118,245p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '2238,2270p' components/river_cloud/river_cloud_adapter.c
rg -n 'uplink_ready_frames\\(|dump_io_status\\(|control queue=|uplink queue=|asr round id=' \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_cloud/river_cloud_internal.h
```
...
- adapter dump-status path no longer formats XiaoZhi control/uplink/ASR-round
  diagnostic lines inline
- adapter transport logic and runtime diagnostics share the same runtime-owned
  retry-aware uplink queue-count helper
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '332,348p' components/river_cloud/river_cloud_internal.h
sed -n '118,194p' components/river_cloud/river_cloud_xiaozhi_session.c
sed -n '1,90p' components/river_cloud/river_cloud_adapter.c
sed -n '1380,1410p' components/river_cloud/river_cloud_adapter.c
rg -n 'river_cloud_xiaozhi_copy_optional_text\\(|river_cloud_xiaozhi_note_preview_observation\\(' \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_cloud/river_cloud_internal.h
```
...
- adapter no longer defines either helper locally
- adapter `input_speech_start / input_preview / input_endpoint` branches still
  delegate to the exported preview helper
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '290,296p' components/river_cloud/river_cloud_adapter.c
sed -n '580,610p' components/river_cloud/river_cloud_adapter.c
sed -n '218,226p' components/river_cloud/river_cloud_xiaozhi_session.c
rg -n 'uplink_send_ready|river_xiaozhi_session_open\\(\\) && g_river_cloud.xiaozhi_listening' \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_session.c \
  components/river_cloud/river_cloud_internal.h
```
...
- build output ends with:
  - `Build done`
- adapter control/uplink paths call:
  - `river_cloud_xiaozhi_uplink_send_ready()`
- session runtime owns that predicate
...
git diff --check
bash -lc 'export AMEBA_SDK_ROOT=/root/ameba-rtos; source /root/ameba-river/env.sh; python3 /root/ameba-rtos/ameba.py build -p'
sed -n '2070,2094p' components/river_cloud/river_cloud_adapter.c
sed -n '312,332p' components/river_cloud/river_cloud_xiaozhi_session.c
rg -n 'xiaozhi_listen_stop_pending|river_cloud_xiaozhi_maybe_start_followup_round' \
  components/river_cloud/river_cloud_adapter.c \
  components/river_cloud/river_cloud_xiaozhi_session.c
```
...
- build output ends with:
  - `Build done`
- adapter no longer contains a reopen-time early return on
  `xiaozhi_listen_stop_pending`
- session runtime `river_cloud_xiaozhi_maybe_start_followup_round(...)` now
  owns that busy gate
