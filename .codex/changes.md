...
## Step 3.2
- Added `river_voice_detector.*` as a dedicated detector boundary after enhancement and before debug replay.
- The first detector implementation is intentionally simple and low-risk:
  - backend name: `energy_vad`
  - input: enhanced mono PCM from `river_voice_preproc`
  - output: speech-active / silence-active decision with hangover
- Current gate policy:
  - speech start requires consecutive active frames
  - speech end uses a hold window to avoid choppy replay
  - silence frames are muted before entering the delayed replay ring
- Updated boot logs and app status so the project now prints:
  - `local_detector=energy_vad`
  - detector tuning baseline during front-end init
- Extended diagnostics with:
  - `vad_avg`
  - `vad=speech|silence`
  - speech-frame and silence-frame counters
- Verified the detector-gated build locally for `RTL8730E`.
