...
int sitronix_ts_get_fw_revision(struct sitronix_ts_data *ts_data)
{
	...
	//Do not return error when getting build version failed.
	sitronix_ts_get_build_version(ts_data);

	stmsg("FW revision (hex) = %x %x %x %x (build: %02x %02x)\n", buf[0], buf[1], buf[2], buf[3],
			ts_data->ts_dev_info.build_version[0], ts_data->ts_dev_info.build_version[1]);
...
int TDU_CmdioSetMemoryArea(unsigned int area)
{
	struct CommandIoPacket outPacket;
	/* CommandIoPacket inPacket; */
	unsigned short chksum = 0;/* , vchksum; */
	int retry = 0;
	area |= ST_READ_FW_OFFSET;
	outPacket.CmdID = 0x01;	/* write RAM/ROM */
	outPacket.ValidDataSize = 6;
	outPacket.CmdData[0] = 1;
	outPacket.CmdData[1] = 0x01;
	outPacket.CmdData[2] = 0x00;
	outPacket.CmdData[3] = 0x01;
	outPacket.CmdData[4] = ((area >> 16) & 0xff);
	...
	if (!TDU_WriteIOCommand(&outPacket)) {
		sterr("%s:%d:TDU_SetDeviceRam: (E)TDU_WriteIOCommand.\n", __func__, __LINE__);
		//goto TDU_CmdioWrite_retry;
		return -1;
		/* return setLen; */
	}
	if (!TDU_SetH2DReady()) {
		sterr("%s:%d:TDU_SetDeviceRam: (E)TDU_SetH2DReady.\n", __func__, __LINE__);
		//goto TDU_CmdioWrite_retry;
		return -1;
		/* return setLen; */
	}
	...
	while (retry < 10) {
		if (!TDU_GetH2DReady()) {
			sterr("%s:%d:TDU_SetDeviceRam: (E)TDU_GetH2DReady.\n", __func__, __LINE__);
			retry++;
...
int TDU_CmdioGetMemoryArea(unsigned char *memory)
{
	struct CommandIoPacket outPacket;
	...
	outPacket.CmdID = 0x02;	/* write RAM/ROM */
	outPacket.ValidDataSize = 5;
	outPacket.CmdData[0] = 1;
	outPacket.CmdData[1] = 0x01;
	outPacket.CmdData[2] = 0x00;
	outPacket.CmdData[3] = 0x01;
	//outPacket.CmdData[4] = area;
	STChecksumCalculation(&chksum, (unsigned char *)&outPacket, outPacket.ValidDataSize + 1);
	outPacket.CmdData[outPacket.ValidDataSize - 1] = (chksum & 0xFF);
	if (!TDU_WriteIOCommand(&outPacket)) {
		sterr("%s:%d:TDU_SetDeviceRam: (E)TDU_WriteIOCommand.\n", __func__, __LINE__);
	...
	//printk("inPacket id = %x, inPacket ValidDataSize = %d\n", inPacket.CmdID, inPacket.ValidDataSize);
	//printk("inPacket CmdData[0] = %x, inPacket CmdData[1] = %x, inPacket CmdData[2] = %x, inPacket CmdData[3] = %x\n", inPacket.CmdData[0], inPacket.CmdData[1], inPacket.CmdData[2], inPacket.CmdData[3]);
	*memory = inPacket.CmdData[2];
	return 0;
}

int TDU_CmdioReadMemoryArea(int area, unsigned char *memory, int len)
{
	struct CommandIoPacket outPacket;
	struct CommandIoPacket inPacket;
	/* CommandIoPacket inPacket; */
	...
	outPacket.CmdID = 0x02;	/* write RAM/ROM */
	outPacket.ValidDataSize = 5;
	outPacket.CmdData[0] = 2;
	outPacket.CmdData[1] = ((area >> 8) & 0xFF);
	outPacket.CmdData[2] = (area & 0xFF);
	outPacket.CmdData[3] = len;
	//outPacket.CmdData[4] = ((area >> 16) & 0xff);
	STChecksumCalculation(&chksum, (unsigned char *)&outPacket, outPacket.ValidDataSize + 1);
	outPacket.CmdData[outPacket.ValidDataSize - 1] = (chksum & 0xFF);
	if (!TDU_WriteIOCommand(&outPacket)) {
	...
	#if 0
	printk("inPacket id = %x, inPacket ValidDataSize = %d\n", inPacket.CmdID, inPacket.ValidDataSize);
	for (i = 0; i < inPacket.ValidDataSize; i++) {
		//memory[i] = inPacket.CmdData[i];
		printk("data[%d] = %02x ", i, inPacket.CmdData[i]);
	}
	printk("\n");
	#endif
	//printk("inPacket CmdData[0] = %x, inPacket CmdData[1] = %x, inPacket CmdData[2] = %x, inPacket CmdData[3] = %x\n", inPacket.CmdData[0], inPacket.CmdData[1], inPacket.CmdData[2], inPacket.CmdData[3]);
	memcpy(memory, inPacket.CmdData+2, len);
	return 0;
}

static int st_get_build_version_84(unsigned char *build_version)
{
	int ret = 0;
	int tdp1_offset = ST_FWBUILDVER_TDP1_OFFSET_84;
	int fwinfotab_offset = 0;
	//unsigned char protocol_ver[2] = {0};
	unsigned char temp_memory[27] = {0};
	#if 0
	ret = sitronix_ts_reg_read(gts, PROTOCOL_VER, protocol_ver, sizeof(protocol_ver));
	printk("%s:%d:protocol ver = %02x %02x\n", __func__, __LINE__, protocol_ver[0], protocol_ver[1]);
	//get "TDP1" offset
	if (((protocol_ver[0]<<8) | protocol_ver[1]) < ST_FWBUILVER_PROTOCOL_VER) {
		sterr("%s:%d:st_get_build_version: (E)protocol version not match.\n", __func__, __LINE__);
		sterr("%s:%d:protocol ver = 0x%x, need 0x121.\n", __func__, __LINE__, (protocol_ver[0]<<8) | protocol_ver[1]);
		//memcpy(build_version, temp_memory, 2);
		//*build_version = 0xff;
		//*(build_version+1) = 0xff;
		//return -EIO;
	}
	#endif
	//get "TDP1" tag
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR+tdp1_offset);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TDP1 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	ret = TDU_CmdioGetMemoryArea(temp_memory);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TDP1 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR+tdp1_offset, temp_memory, 4);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TDP1 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	if (strncmp((const char *)temp_memory, "TDP1", 4) != 0) {
		sterr("%s:%d:st_get_build_version: (E)TDP1 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	} else {
		stmsg("get TDP1\n");
	...
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR+tdp1_offset+ST_FWINFOTAB_ADDR_OFFSET_84);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)FWINFOTAB_ADDR_OFFSET not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	ret = TDU_CmdioGetMemoryArea(temp_memory);
	if (ret != 0) {
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR+tdp1_offset+ST_FWINFOTAB_ADDR_OFFSET, temp_memory, 2);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)FWINFOTAB_ADDR_OFFSET not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	//printk("temp_memory[0] = %02x, temp_memory[1] = %02x\n", temp_memory[0], temp_memory[1]);
	fwinfotab_offset = temp_memory[0] << 8 | temp_memory[1];

	//get "TFI3" tag
	//fwinfotab_offset -= 0x10000;//large size, store in part2
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR+fwinfotab_offset);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	ret = TDU_CmdioGetMemoryArea(temp_memory);
	if (ret != 0) {
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR+fwinfotab_offset, temp_memory, 4);
	//printk("temp_memory[0] = %02x, temp_memory[1] = %02x, temp_memory[2] = %02x, temp_memory[3] = %02x\n", temp_memory[0], temp_memory[1], temp_memory[2], temp_memory[3]);
	if (strncmp((const char *)temp_memory, "TFI3", 4) != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	} else {
		stmsg("get TFI3\n");
	...
	//get build version
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR+fwinfotab_offset+ST_FWBUILDVER_ADDR_OFFSET_84);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR+fwinfotab_offset+ST_FWBUILDVER_ADDR_OFFSET_84, temp_memory, 2);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	...
	memcpy(build_version, temp_memory, 2);
	return 0;
get_buildversion_error:
	*build_version = 0xff;
	*(build_version+1) = 0xff;
	return -EIO;
}

static int st_get_build_version_default(unsigned char *build_version)
{
	int ret = 0;
	int tdp1_offset = 0;
	int fwinfotab_offset = 0;
	...
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR+ST_FW_CRC_ADDR_OFFSET);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)CRC not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	ret = TDU_CmdioGetMemoryArea(temp_memory);
	if (ret != 0) {
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR+ST_FW_CRC_ADDR_OFFSET, temp_memory, 4);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)CRC not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	//printk("temp_memory[0] = %02x, temp_memory[1] = %02x, temp_memory[2] = %02x, temp_memory[3] = %02x\n", temp_memory[0], temp_memory[1], temp_memory[2], temp_memory[3]);
	if (temp_memory[1] == 0x01) {
		tdp1_offset = 4;
	} else {
		tdp1_offset = 2;
	}
	//get "TDP1" tag
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR+ST_FW_CRC_ADDR_OFFSET+tdp1_offset);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TDP1 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR+ST_FW_CRC_ADDR_OFFSET+tdp1_offset, temp_memory, 4);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TDP1 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	...
	//get "FWINFOTAB_ADDR_OFFSET" offset
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR+ST_FW_CRC_ADDR_OFFSET+tdp1_offset+ST_FWINFOTAB_ADDR_OFFSET);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)FWINFOTAB_ADDR_OFFSET not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR+ST_FW_CRC_ADDR_OFFSET+tdp1_offset+ST_FWINFOTAB_ADDR_OFFSET, temp_memory, 4);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)FWINFOTAB_ADDR_OFFSET not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	//printk("temp_memory[0] = %02x, temp_memory[1] = %02x, temp_memory[2] = %02x, temp_memory[3] = %02x\n", temp_memory[0], temp_memory[1], temp_memory[2], temp_memory[3]);
	fwinfotab_offset = temp_memory[0] << 24 | temp_memory[1] << 16 | temp_memory[2] << 8 | temp_memory[3];

	//get "TFI3" tag
	fwinfotab_offset -= 0x10000;//large size, store in part2
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR_PART2+fwinfotab_offset);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR_PART2+fwinfotab_offset, temp_memory, 4);
	//printk("temp_memory[0] = %02x, temp_memory[1] = %02x, temp_memory[2] = %02x, temp_memory[3] = %02x\n", temp_memory[0], temp_memory[1], temp_memory[2], temp_memory[3]);
	if (strncmp((const char *)temp_memory, "TFI3", 4) != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	...
	//get build version
	memset(temp_memory, 0, 27);
	ret = TDU_CmdioSetMemoryArea(ST_HDL_PRAM_ADDR_PART2+fwinfotab_offset+ST_FWBUILDVER_ADDR_OFFSET);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
	...
	ret = TDU_CmdioReadMemoryArea(ST_HDL_PRAM_ADDR_PART2+fwinfotab_offset+ST_FWBUILDVER_ADDR_OFFSET, temp_memory, 2);
	if (ret != 0) {
		sterr("%s:%d:st_get_build_version: (E)TFI3 not found.\n", __func__, __LINE__);
		goto get_buildversion_error;
	}
...
int sitronix_ts_get_build_version(struct sitronix_ts_data *ts_data)
{
	int ret;

	if(ts_data->ts_dev_info.chip_id == 0x84) {
		ret = st_get_build_version_84(ts_data->ts_dev_info.build_version);
	} else {
		ret = st_get_build_version_default(ts_data->ts_dev_info.build_version);
	}
...
int sitronix_checkesd(void)
{
	...
	if (gts->ts_dev_info.chip_id == 0x84){	
		...
		off1 = sitronix_ts_addrmode_read(gts, wbuf, 3, temp, 3);
		if (off1 < 0) {
			sterr("read ESD fail \n");
			ret = -1;
		}
		else{
			stmsg("ESD_GOLDEN: 0x%X 0x%X \n", temp[off1+1], temp[off1+2]);
			ret = 1;
		}
		...
		wbuf[2] = 0xF1;
		wbuf[3] = 0x92;
		off2 = sitronix_ts_addrmode_read(gts, wbuf, 3, temp, 3);
		...
		if (off2 < 0) {
			...
			stmsg("ESD_SUM: 0x%X 0x%X \n", temp[off2+1], temp[off2+2]);
			ret = 1;
		}
	}else{
		wbuf[1] = 0x80;
...
int sitronix_read_driver_cmd(unsigned char dc, unsigned char *buf, int len)
{
	...
	off = sitronix_ts_addrmode_split_read(gts, cmd, 3, wbuf, len+1);	//len+1 is for dummy after 3 address bytes.

	if (off < 0) {
		sterr("read display CMD1 fail - 2\n");		
	}
	memcpy(buf, wbuf + off + 1, len);	//wbuf+off+1 to skip dummy byte.
