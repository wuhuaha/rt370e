...
static river_status_t river_cloud_xiaozhi_control_request(
    ...
    const char *arg);
static river_status_t river_cloud_xiaozhi_control_request_async(
    river_cloud_xiaozhi_control_op_t op,
    const char *arg,
...
static void river_cloud_xiaozhi_try_queue_playback_cleared_ack(const char *reason);
static void river_cloud_xiaozhi_try_queue_playback_completed_ack(void);
static void river_cloud_xiaozhi_update_playback_ack_progress(void);
static void river_cloud_xiaozhi_finalize_playback_cleared(const char *reason);
static uint32_t river_cloud_xiaozhi_downlink_ready_frames(void);
...
static void river_cloud_xiaozhi_note_playback_meta(const river_xiaozhi_event_t *event)
{
...
static void river_cloud_xiaozhi_finish_playback_rebuffer(uint64_t resumed_at_ms)
{
    ...
    if (segment != NULL && segment->valid && segment->started && segment->paused_at_ms != 0U &&
        ...
        RIVER_LOGW("xiaozhi playback rebuffer resumed: pause_ms=%lu queued=%lu count=%lu",
                   ...
                   (unsigned long)river_cloud_xiaozhi_downlink_ready_frames(),
...
static uint32_t river_cloud_xiaozhi_trim_uplink_stale_frames(uint32_t keep_frames)
{
    ...
    while (river_audio_frame_ring_count(&g_river_cloud.xiaozhi_uplink_ring) > keep_frames) {
...
static bool river_cloud_xiaozhi_duplex_soft_endpoint_enabled(void)
{
...
static void river_cloud_xiaozhi_cancel_endpoint_soft_close(const char *trigger)
{
...
static void river_cloud_xiaozhi_note_interrupt_hint(const char *trigger,
                                                    const char *reason)
{
...
static void river_cloud_xiaozhi_arm_endpoint_soft_close(const char *trigger,
                                                        const char *reason)
{
    if (!river_cloud_xiaozhi_duplex_soft_endpoint_enabled() ||
...
static bool river_cloud_xiaozhi_should_defer_local_close(void)
{
...
static void river_cloud_xiaozhi_arm_local_close_defer(void)
{
...
static void river_cloud_xiaozhi_resolve_local_close(const char *trigger)
{
    if (!g_river_cloud.xiaozhi_local_close_pending ||
        g_river_cloud.xiaozhi_listen_stop_pending) {
        return;
    ...
    RIVER_LOGI("xiaozhi local close resolved: trigger=%s partial=%s final=%s",
               trigger != NULL ? trigger : "-",
...
static void river_cloud_xiaozhi_check_endpoint_soft_close_timeout(void)
{
    uint64_t now_ms;
...
static void river_cloud_xiaozhi_check_local_close_timeout(void)
{
    ...
    river_cloud_xiaozhi_resolve_local_close("timeout");
}
...
static void river_cloud_xiaozhi_round_finish(const char *reason)
{
    ...
    RIVER_LOGI("xiaozhi asr round finish: id=%lu sid=%s reason=%s duration_ms=%lu first_packet_delay_ms=%lu pre_roll_frames=%lu packets=%lu busy=%lu fail=%lu stale_drop=%lu ring_drop=%lu partial=%lu final=%lu seen[partial=%s final=%s]",
               (unsigned long)g_river_cloud.xiaozhi_asr_round_id,
               river_cloud_xiaozhi_current_sid() != NULL ?
                   river_cloud_xiaozhi_current_sid() :
                   "-",
...
static void river_cloud_xiaozhi_finalize_playback_cleared(const char *reason)
{
    ...
    if (g_river_cloud.xiaozhi_playback_last_fully_heard_segment_id[0] != '\0') {
        river_cloud_xiaozhi_try_queue_playback_cleared_ack(reason);
    }
...
static void river_cloud_xiaozhi_io_task(void *arg)
{
    ...
    for (;;) {
        ...
        river_cloud_xiaozhi_check_endpoint_soft_close_timeout();
...
static void river_cloud_xiaozhi_downlink_task(void *arg)
{
    ...
    for (;;) {
        ...
        queued_frames = river_cloud_xiaozhi_downlink_ready_frames();
        if (queued_frames == 0U) {
            river_cloud_xiaozhi_update_playback_ack_progress();
            river_cloud_xiaozhi_check_pending_playback_stop();
        ...
        if (stereo_bytes > sizeof(g_river_cloud.xiaozhi_downlink_stereo)) {
            river_cloud_xiaozhi_finalize_playback_cleared("frame_oversize");
...
static void river_cloud_xiaozhi_check_pending_playback_stop(void)
{
    ...
    if (!river_playback_service_active()) {
        if (river_cloud_xiaozhi_downlink_ready_frames() != 0U) {
            return;
        }
        g_river_cloud.xiaozhi_playback_rebuffer_pending = false;
...
static void river_cloud_xiaozhi_finalize_listen_stop_if_ready(void)
{
    ...
    if (g_river_cloud.xiaozhi_local_close_pending) {
        ...
        if (g_river_cloud.xiaozhi_pending_text_finalized ||
            g_river_cloud.xiaozhi_asr_round_final_seen) {
            river_cloud_xiaozhi_resolve_local_close("post_stop_result");
        }
        return;
    }
...
static void river_cloud_xiaozhi_event_handler(const river_xiaozhi_event_t *event, void *user_data)
{
    ...
    case RIVER_XIAOZHI_EVENT_AUDIO_OUT_META:
        river_cloud_xiaozhi_window_touch(RIVER_CLOUD_XIAOZHI_WAKE_WINDOW_FOLLOWUP_MS,
                                         "audio_out_meta");
        river_cloud_xiaozhi_note_playback_meta(event);
    ...
    case RIVER_XIAOZHI_EVENT_LLM:
        ...
        river_cloud_xiaozhi_resolve_local_close("llm");
        break;
    case RIVER_XIAOZHI_EVENT_TTS:
        if (event->state != NULL && strcmp(event->state, "start") == 0) {
            ...
            river_cloud_xiaozhi_resolve_local_close("tts_stop");
    ...
    case RIVER_XIAOZHI_EVENT_SESSION_CLOSED:
        ...
        if (g_river_cloud.xiaozhi_playback_active || g_river_cloud.xiaozhi_tts_stop_pending ||
            ...
            g_river_cloud.xiaozhi_playback_meta_valid) {
            river_cloud_xiaozhi_finalize_playback_cleared("transport_closed");
            river_cloud_xiaozhi_reset_downlink_state();
            if (river_playback_service_active()) {
                (void)river_playback_service_stop_stream_ex("xiaozhi_transport_closed");
            }
            river_cloud_xiaozhi_reset_playback_state();
        }
...
void river_cloud_adapter_notify_network_lost(void)
{
    ...
    if (river_cloud_xiaozhi_enabled()) {
        if (g_river_cloud.xiaozhi_playback_active || g_river_cloud.xiaozhi_tts_stop_pending ||
            river_audio_frame_ring_count(&g_river_cloud.xiaozhi_downlink_ring) != 0U ||
            g_river_cloud.xiaozhi_playback_meta_valid) {
            river_cloud_xiaozhi_finalize_playback_cleared("network_lost");
        }
        if (g_river_cloud.xiaozhi_playback_active && river_playback_service_active()) {
...
river_status_t river_cloud_adapter_interrupt_tts_with_reason(const char *reason)
{
    ...
    if (river_cloud_xiaozhi_enabled() &&
        ...
        if (g_river_cloud.xiaozhi_playback_active || g_river_cloud.xiaozhi_tts_stop_pending ||
            river_audio_frame_ring_count(&g_river_cloud.xiaozhi_downlink_ring) != 0U ||
            g_river_cloud.xiaozhi_playback_meta_valid) {
            river_cloud_xiaozhi_finalize_playback_cleared(reason != NULL ? reason :
                                                              "xiaozhi_interrupt");
            river_cloud_xiaozhi_reset_downlink_state();
        }
        if (river_playback_service_active()) {
...
void river_cloud_asr_audio_close(void)
{
    ...
    if (river_cloud_xiaozhi_enabled()) {
        ...
        if (g_river_cloud.xiaozhi_playback_active || g_river_cloud.xiaozhi_tts_stop_pending ||
            river_audio_frame_ring_count(&g_river_cloud.xiaozhi_downlink_ring) != 0U ||
            g_river_cloud.xiaozhi_playback_meta_valid) {
            river_cloud_xiaozhi_finalize_playback_cleared("bridge_close");
            if (river_playback_service_active()) {
                (void)river_playback_service_stop_stream_ex("xiaozhi_bridge_close");
            }
        }
                                                            ...
                                                            bool is_speech)
{
    ...
    if (!g_river_cloud.stream_active) {
        ...
        if (g_river_cloud.xiaozhi_local_close_pending) {
            river_cloud_xiaozhi_round_request_finish("reopen_overlap");
            river_cloud_xiaozhi_resolve_local_close("reopen_overlap");
        }
