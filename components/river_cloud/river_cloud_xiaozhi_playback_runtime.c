...
{
    return g_river_cloud.xiaozhi_playback_wait_response_id[0] != '\0' &&
           g_river_cloud.xiaozhi_playback_wait_playback_id[0] != '\0' &&
           g_river_cloud.xiaozhi_playback_wait_segment_id[0] != '\0';
}
...
{
    river_cloud_xiaozhi_playback_phase_source_t source;
    river_cloud_playback_phase_t old_phase = g_river_cloud.xiaozhi_playback_phase;
...
{
    return g_river_cloud.xiaozhi_playback_terminal_state_kind ==
    ...
    river_cloud_playback_terminal_wait_kind_t kind)
{
    ...
    g_river_cloud.xiaozhi_playback_terminal_wait_kind = kind;
...
{
    river_cloud_xiaozhi_clear_playback_terminal_wait();
    g_river_cloud.xiaozhi_playback_terminal_state_kind = state;
...
static bool river_cloud_xiaozhi_playback_last_segment_observed(void)
{
    return g_river_cloud.xiaozhi_playback_last_segment_response_id[0] != '\0' &&
           g_river_cloud.xiaozhi_playback_last_segment_playback_id[0] != '\0' &&
           g_river_cloud.xiaozhi_playback_last_segment_segment_id[0] != '\0';
}

static bool river_cloud_xiaozhi_playback_response_context_valid(void)
{
    return g_river_cloud.xiaozhi_playback_response_id[0] != '\0' &&
           g_river_cloud.xiaozhi_playback_id[0] != '\0';
}

static bool river_cloud_xiaozhi_playback_segment_context_valid(void)
{
    return river_cloud_xiaozhi_playback_response_context_valid() &&
...
static bool river_cloud_xiaozhi_playback_current_meta_is_last_segment(void)
{
    ...
    return strcmp(g_river_cloud.xiaozhi_playback_response_id,
                  g_river_cloud.xiaozhi_playback_last_segment_response_id) == 0 &&
           strcmp(g_river_cloud.xiaozhi_playback_id,
                  g_river_cloud.xiaozhi_playback_last_segment_playback_id) == 0 &&
           strcmp(g_river_cloud.xiaozhi_playback_segment_id,
                  g_river_cloud.xiaozhi_playback_last_segment_segment_id) == 0;
}

static void river_cloud_xiaozhi_clear_playback_wait_context(void)
{
    g_river_cloud.xiaozhi_playback_wait_response_id[0] = '\0';
    g_river_cloud.xiaozhi_playback_wait_playback_id[0] = '\0';
    g_river_cloud.xiaozhi_playback_wait_segment_id[0] = '\0';
}
...
{
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_wait_response_id,
        sizeof(g_river_cloud.xiaozhi_playback_wait_response_id),
        g_river_cloud.xiaozhi_playback_response_id);
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_wait_playback_id,
        sizeof(g_river_cloud.xiaozhi_playback_wait_playback_id),
        g_river_cloud.xiaozhi_playback_id);
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_wait_segment_id,
        sizeof(g_river_cloud.xiaozhi_playback_wait_segment_id),
...
{
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_last_segment_response_id,
        sizeof(g_river_cloud.xiaozhi_playback_last_segment_response_id),
        g_river_cloud.xiaozhi_playback_response_id);
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_last_segment_playback_id,
        sizeof(g_river_cloud.xiaozhi_playback_last_segment_playback_id),
        g_river_cloud.xiaozhi_playback_id);
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_last_segment_segment_id,
        sizeof(g_river_cloud.xiaozhi_playback_last_segment_segment_id),
        g_river_cloud.xiaozhi_playback_segment_id);
}
...
typedef enum {
    RIVER_CLOUD_XIAOZHI_DOWNLINK_PREPARE_RETRY_LATER = 0,
    RIVER_CLOUD_XIAOZHI_DOWNLINK_PREPARE_READY
} river_cloud_xiaozhi_downlink_prepare_result_t;

typedef enum {
    RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_SLEEP_IDLE = 0,
    RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_SLEEP_POLL,
    RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_READY
...
{
    return g_river_cloud.xiaozhi_endpoint_soft_close_pending;
}
...
uint64_t river_cloud_xiaozhi_endpoint_soft_close_remaining_ms(uint64_t now_ms)
{
    ...
    return g_river_cloud.xiaozhi_endpoint_soft_close_deadline_ms - now_ms;
}

const char *river_cloud_xiaozhi_endpoint_soft_close_reason(void)
{
    return g_river_cloud.xiaozhi_endpoint_soft_close_reason[0] != '\0' ?
...
static river_cloud_xiaozhi_playback_start_gate_t river_cloud_xiaozhi_build_start_gate(void)
{
    ...
    if (g_river_cloud.xiaozhi_playback_rebuffer_streak > 1U) {
        uint32_t extra_frames =
            (g_river_cloud.xiaozhi_playback_rebuffer_streak - 1U) *
...
static river_cloud_xiaozhi_playback_start_gate_t river_cloud_xiaozhi_current_start_gate(void)
{
    ...
    memset(&gate, 0, sizeof(gate));
    if (g_river_cloud.xiaozhi_playback_start_frames == 0U) {
...
static river_status_t river_cloud_xiaozhi_execute_managed_playback_rebuffer_recovery(
    const char *reason,
    ...
    const char **recover_path_out)
{
    if (recover_path_out != NULL) {
        *recover_path_out = "stop_rebuffer";
    }
    if (plan == NULL) {
    ...
    return river_cloud_xiaozhi_request_playback_rebuffer_recovery(
        reason,
        plan->cause,
        plan->truth_view.supply_kind,
        recover_path_out);
    ...
    const char *recover_path)
{
    ...
    RIVER_LOGW("%s: cause=%s supply=%s phase=%s backend=%s queued=%lu low=%u start=%u policy=%s cautious=%s target_ms=%u total=%lu streak=%lu recovery=%s",
               ...
               (unsigned int)recovery_plan->start_gate.start_frames,
               river_cloud_playback_start_policy_name(
                   recovery_plan->start_gate.policy),
               recovery_plan->start_gate.cautious_history ? "yes" : "no",
               ...
               recover_path != NULL ? recover_path : "stop_rebuffer");
}

static void river_cloud_xiaozhi_handle_write_failed_managed_rebuffer(
    ...
    bool force_stop_rebuffer,
    const char **recover_path_io)
{
    if (recovery_plan == NULL) {
        return;
    }

    river_cloud_xiaozhi_begin_managed_playback_rebuffer(recovery_plan, now_ms, true);
    if (recover_path_io != NULL && force_stop_rebuffer) {
        *recover_path_io = "stop_rebuffer";
    }
    river_cloud_xiaozhi_log_write_failed_rebuffer_request(
        request_log,
        recovery_plan,
        queued_frames,
        recover_path_io != NULL ? *recover_path_io : NULL);
    if (river_cloud_xiaozhi_execute_managed_playback_rebuffer_recovery(
            recover_reason,
            recovery_plan,
            force_stop_rebuffer,
            recover_path_io) != RIVER_OK) {
    ...
    const char *recover_path)
{
    ...
    RIVER_LOGW("xiaozhi playback service recover requested: cause=%s supply=%s phase=%s backend=%s queued=%lu low=%u start=%u policy=%s cautious=%s target_ms=%u total=%lu streak=%lu recovery=%s",
               ...
               (unsigned long)g_river_cloud.xiaozhi_playback_rebuffer_count,
               (unsigned long)g_river_cloud.xiaozhi_playback_rebuffer_streak,
               recover_path != NULL ? recover_path : "service_recover");
...
void river_cloud_xiaozhi_dump_playback_status(uint64_t now_ms)
{
    ...
    river_cloud_xiaozhi_capture_playback_truth_view(&truth_view);
    start_gate = river_cloud_xiaozhi_current_start_gate();
    river_cloud_xiaozhi_get_duplex_ready_eval(&duplex_eval);
    ...
    RIVER_LOGI("xiaozhi playback_heard_context response_id=%s playback_id=%s segment_id=%s valid=%s",
               g_river_cloud.xiaozhi_playback_last_fully_heard_response_id[0] != '\0' ?
                   g_river_cloud.xiaozhi_playback_last_fully_heard_response_id :
                   "-",
               g_river_cloud.xiaozhi_playback_last_fully_heard_playback_id[0] != '\0' ?
                   g_river_cloud.xiaozhi_playback_last_fully_heard_playback_id :
                   "-",
               g_river_cloud.xiaozhi_playback_last_fully_heard_segment_id[0] != '\0' ?
                   g_river_cloud.xiaozhi_playback_last_fully_heard_segment_id :
                   "-",
    ...
    RIVER_LOGI("xiaozhi playback_terminal_context response_id=%s playback_id=%s segment_id=%s valid=%s",
               g_river_cloud.xiaozhi_playback_last_segment_response_id[0] != '\0' ?
                   g_river_cloud.xiaozhi_playback_last_segment_response_id :
                   "-",
               g_river_cloud.xiaozhi_playback_last_segment_playback_id[0] != '\0' ?
                   g_river_cloud.xiaozhi_playback_last_segment_playback_id :
    ...
    RIVER_LOGI("xiaozhi downlink queue=%lu/%u dropped=%lu worker=%s sample=%luHz frame=%lums start=%u resume=%u buffer=%u policy=%s cautious=%s target_ms=%lu prefetch_frames=%u meta_gap_ms=%lu supply=%s phase=%s backend=%s hold=%s rebuffer=%s/%s recovery_path=%s recovery_outcome=%s rebuffer_total=%lu streak=%lu",
               ...
               (unsigned int)river_cloud_xiaozhi_downlink_attached_resume_threshold_frames(),
               ...
               river_cloud_playback_recovery_path_name(
                   river_cloud_xiaozhi_playback_recovery_path()) != NULL ?
                   river_cloud_playback_recovery_path_name(
                       river_cloud_xiaozhi_playback_recovery_path()) :
                   "-",
               river_cloud_playback_recovery_outcome_name(
                   river_cloud_xiaozhi_playback_recovery_outcome()) != NULL ?
                   river_cloud_playback_recovery_outcome_name(
                       river_cloud_xiaozhi_playback_recovery_outcome()) :
                   "-",
               (unsigned long)g_river_cloud.xiaozhi_playback_rebuffer_count,
               (unsigned long)g_river_cloud.xiaozhi_playback_rebuffer_streak);
}
    ...
    river_cloud_runtime_snapshot_t *snapshot)
{
    ...
    river_cloud_xiaozhi_capture_playback_truth_view(&truth_view);
    river_cloud_xiaozhi_capture_playback_observe_view(&observe_view);
    snapshot->playback_active = truth_view.output_active;
    snapshot->playback_lane_engaged =
        river_cloud_xiaozhi_playback_lane_engaged_from_truth_view(&truth_view);
    snapshot->playback_turn_active =
        river_cloud_xiaozhi_playback_turn_active_from_truth_view(&truth_view);
    snapshot->playback_rebuffer_pending = truth_view.rebuffer_pending;
    snapshot->playback_phase_known = observe_view.phase_known;
    snapshot->playback_backend_state_kind = truth_view.backend_state;
    snapshot->playback_supply_kind = truth_view.supply_kind;
    snapshot->playback_hold_kind = truth_view.hold_kind;
    ...
    snapshot->playback_phase_kind = observe_view.phase_kind;
    ...
    snapshot->playback_rebuffer_cause_kind =
        river_cloud_xiaozhi_playback_rebuffer_cause();
    snapshot->playback_recovery_path_kind = observe_view.recovery_path_kind;
    snapshot->playback_recovery_outcome_kind = observe_view.recovery_outcome_kind;
    ...
    river_cloud_xiaozhi_copy_optional_text(
        ...
        observe_view.recovery_path_kind !=
    ...
    river_cloud_xiaozhi_copy_optional_text(
        ...
        observe_view.recovery_outcome_kind !=
    ...
    river_cloud_xiaozhi_copy_optional_text(snapshot->playback_terminal_state,
                                           ...
                                           g_river_cloud.xiaozhi_playback_terminal_state_kind !=
                                               ...
                                               river_cloud_playback_terminal_state_name(
                                                   g_river_cloud
                                                       .xiaozhi_playback_terminal_state_kind) :
                                               NULL);
...
void river_cloud_xiaozhi_clear_playback_meta_state(void)
{
    ...
    g_river_cloud.xiaozhi_playback_rebuffer_cause =
        RIVER_CLOUD_PLAYBACK_REBUFFER_CAUSE_NONE;
    g_river_cloud.xiaozhi_playback_recovery_path =
        RIVER_CLOUD_PLAYBACK_RECOVERY_PATH_NONE;
    g_river_cloud.xiaozhi_playback_recovery_outcome =
        RIVER_CLOUD_PLAYBACK_RECOVERY_OUTCOME_NONE;
    ...
    g_river_cloud.xiaozhi_playback_buffer_frames = 0U;
    g_river_cloud.xiaozhi_playback_last_meta_ms = 0U;
...
static uint32_t river_cloud_xiaozhi_playback_buffer_frame_budget(void)
{
    if (g_river_cloud.xiaozhi_playback_buffer_frames != 0U) {
        return g_river_cloud.xiaozhi_playback_buffer_frames;
    }
    ...
    river_cloud_playback_rebuffer_cause_t cause)
{
    ...
    if (g_river_cloud.xiaozhi_playback_rebuffer_streak < UINT32_MAX) {
        g_river_cloud.xiaozhi_playback_rebuffer_streak++;
    }
    (void)river_cloud_xiaozhi_refresh_start_gate();
    ...
    const char *reason)
{
    ...
    g_river_cloud.xiaozhi_playback_recovery_path = path;
    if (old_path != path) {
        river_cloud_request_state_sync(reason != NULL && reason[0] != '\0' ? reason :
                                                                              "playback_recovery_path");
    }
    ...
    const char *reason)
{
    ...
    g_river_cloud.xiaozhi_playback_recovery_outcome = outcome;
                                                          ...
                                                          backend_state)
{
    ...
    if (!g_river_cloud.xiaozhi_playback_rebuffer_pending || g_river_cloud.xiaozhi_tts_stop_pending) {
        return true;
    }
    ...
    const river_cloud_xiaozhi_playback_segment_t *segment)
{
    ...
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_last_fully_heard_response_id,
        sizeof(g_river_cloud.xiaozhi_playback_last_fully_heard_response_id),
        segment->response_id);
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_last_fully_heard_playback_id,
        sizeof(g_river_cloud.xiaozhi_playback_last_fully_heard_playback_id),
        segment->playback_id);
    river_cloud_xiaozhi_copy_optional_text(
        g_river_cloud.xiaozhi_playback_last_fully_heard_segment_id,
...
static bool river_cloud_xiaozhi_playback_last_fully_heard_context_valid(void)
{
    return g_river_cloud.xiaozhi_playback_last_fully_heard_response_id[0] != '\0' &&
           g_river_cloud.xiaozhi_playback_last_fully_heard_playback_id[0] != '\0' &&
           g_river_cloud.xiaozhi_playback_last_fully_heard_segment_id[0] != '\0';
}
...
void river_cloud_xiaozhi_playback_note_meta(const river_xiaozhi_event_t *event)
{
    ...
    now_ms = (uint64_t)rtos_time_get_current_system_time_ms();
    prefetch_target_ms = g_river_cloud.xiaozhi_playback_expected_duration_ms;
    if (prefetch_target_ms == 0U) {
        prefetch_target_ms = frame_ms * RIVER_CLOUD_XIAOZHI_DOWNLINK_START_FRAMES;
    }
    if (g_river_cloud.xiaozhi_playback_last_meta_ms != 0U &&
        now_ms > g_river_cloud.xiaozhi_playback_last_meta_ms) {
        uint64_t meta_gap_ms = now_ms - g_river_cloud.xiaozhi_playback_last_meta_ms;

        if (meta_gap_ms > UINT32_MAX) {
            meta_gap_ms = UINT32_MAX;
        }
        g_river_cloud.xiaozhi_playback_last_meta_gap_ms = (uint32_t)meta_gap_ms;
        if (g_river_cloud.xiaozhi_playback_last_meta_gap_ms > prefetch_target_ms) {
            prefetch_target_ms = g_river_cloud.xiaozhi_playback_last_meta_gap_ms;
        }
    } else {
        g_river_cloud.xiaozhi_playback_last_meta_gap_ms = 0U;
    }
...
static bool river_cloud_xiaozhi_try_queue_playback_cleared_ack(const char *reason)
{
    ...
    response_id = g_river_cloud.xiaozhi_playback_last_fully_heard_response_id;
    playback_id = g_river_cloud.xiaozhi_playback_last_fully_heard_playback_id;
    segment_id = g_river_cloud.xiaozhi_playback_last_fully_heard_segment_id;
    status = river_cloud_xiaozhi_control_request_async(
...
static bool river_cloud_xiaozhi_try_queue_playback_completed_ack(void)
{
    ...
    response_id = g_river_cloud.xiaozhi_playback_last_segment_response_id;
    playback_id = g_river_cloud.xiaozhi_playback_last_segment_playback_id;
                                                                   ...
                                                                   size_t mono_bytes)
{
    ...
    g_river_cloud.xiaozhi_playback_buffer_frames = buffer_frames;
...
static river_cloud_xiaozhi_downlink_prepare_result_t
river_cloud_xiaozhi_prepare_downlink_playback(uint32_t queued_frames, uint64_t now_ms)
{
    ...
    if (!river_cloud_xiaozhi_rebuffer_resume_ready(queued_frames,
                                                   now_ms,
                                                   truth_view.backend_state)) {
        return RIVER_CLOUD_XIAOZHI_DOWNLINK_PREPARE_RETRY_LATER;
    }
    if (g_river_cloud.xiaozhi_tts_stop_pending &&
    ...
    if (truth_view.backend_state == RIVER_CLOUD_PLAYBACK_BACKEND_OWNED_ACTIVE) {
        return RIVER_CLOUD_XIAOZHI_DOWNLINK_PREPARE_READY;
    }
    if (truth_view.backend_state == RIVER_CLOUD_PLAYBACK_BACKEND_OWNED_RECOVERING) {
        return RIVER_CLOUD_XIAOZHI_DOWNLINK_PREPARE_RETRY_LATER;
    }
    ...
    return river_cloud_xiaozhi_start_playback_if_needed(
               ...
               RIVER_CLOUD_XIAOZHI_DOWNLINK_PREPARE_READY :
               RIVER_CLOUD_XIAOZHI_DOWNLINK_PREPARE_RETRY_LATER;
}

static bool river_cloud_xiaozhi_downlink_active(void);

static river_cloud_xiaozhi_downlink_cycle_result_t
river_cloud_xiaozhi_prepare_downlink_cycle(uint32_t *queued_frames_out)
{
    uint32_t queued_frames;
    uint64_t now_ms;

    if (!river_cloud_xiaozhi_downlink_active()) {
        return RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_SLEEP_IDLE;
    }
    ...
    if (river_cloud_xiaozhi_maybe_rebuffer_starved(queued_frames, now_ms)) {
        return RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_SLEEP_POLL;
    }
    ...
    if (queued_frames_out != NULL) {
        *queued_frames_out = queued_frames;
    }
    return RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_READY;
}
...
{
    uint32_t queued_frames;
    river_cloud_xiaozhi_downlink_cycle_result_t cycle_result;
    river_cloud_xiaozhi_downlink_execute_result_t execute_result;

    (void)arg;

    for (;;) {
        cycle_result = river_cloud_xiaozhi_prepare_downlink_cycle(&queued_frames);
        if (cycle_result == RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_SLEEP_IDLE) {
            rtos_time_delay_ms(RIVER_CLOUD_XIAOZHI_DOWNLINK_IDLE_MS);
            continue;
        }
        if (cycle_result != RIVER_CLOUD_XIAOZHI_DOWNLINK_CYCLE_READY) {
            rtos_time_delay_ms(RIVER_CLOUD_XIAOZHI_DOWNLINK_POLL_MS);
            continue;
        }

        execute_result = river_cloud_xiaozhi_execute_ready_downlink_cycle(queued_frames);
        if (execute_result == RIVER_CLOUD_XIAOZHI_DOWNLINK_EXECUTE_SLEEP_POLL) {
            rtos_time_delay_ms(RIVER_CLOUD_XIAOZHI_DOWNLINK_POLL_MS);
        }
    }
}
