...
static bool river_cloud_xiaozhi_playback_waiting_next_segment(void)
{
    return river_cloud_xiaozhi_playback_wait_context_valid() &&
           g_river_cloud.xiaozhi_playback_segment_count == 0U;
}

static river_cloud_playback_phase_t river_cloud_xiaozhi_compute_playback_phase(void)
{
...
static bool river_cloud_xiaozhi_refresh_playback_phase(const char *reason)
{
    ...
    RIVER_LOGI("xiaozhi playback phase: %s -> %s reason=%s queued=%lu active=%s backend=%s rebuffer=%s cause=%s stop=%s",
...
void river_cloud_xiaozhi_dump_playback_status(uint64_t now_ms)
{
    ...
    RIVER_LOGI("xiaozhi duplex_ready=%s reason=%s aec=%s ref_state=%s ref_activity=%s ref_peak=%u ref_ratio_q15=%u ref_queue=%lu/%lu ref_age_ms=%lu playback_active=%s backend=%s duplex_seen=%s",
               duplex_eval.ready ? "yes" : "no",
               river_voice_runtime_duplex_ready_reason_name(duplex_eval.reason),
               river_voice_runtime_aec_gate_reason_name(duplex_eval.aec_reason),
               river_reference_service_state_name(duplex_eval.reference_state),
    ...
    RIVER_LOGI("xiaozhi downlink queue=%lu/%u dropped=%lu worker=%s sample=%luHz frame=%lums start=%u resume=%u buffer=%u policy=%s cautious=%s target_ms=%lu prefetch_frames=%u meta_gap_ms=%lu backend=%s rebuffer=%s/%s rebuffer_total=%lu streak=%lu",
...
static bool river_cloud_xiaozhi_maybe_pause_for_segment_gap(void)
{
    ...
    RIVER_LOGI("xiaozhi playback segment gap hold: response_id=%s playback_id=%s segment_id=%s phase=%s backend=%s mode=%s",
               g_river_cloud.xiaozhi_playback_response_id[0] != '\0' ?
                   g_river_cloud.xiaozhi_playback_response_id :
                   "-",
               g_river_cloud.xiaozhi_playback_id[0] != '\0' ?
...
static bool river_cloud_xiaozhi_maybe_resume_paused_playback(uint32_t queued_frames)
{
    uint32_t start_frames;
    uint32_t resume_frames;
    ...
    const char *detail_reason)
{
    ...
    RIVER_LOGI("xiaozhi playback abort: cause=%s clear_reason=%s stream_reason=%s interrupt=%s had_work=%s stream_attached=%s backend=%s",
                                                                   ...
                                                                   size_t mono_bytes)
{
    ...
    if (backend_state == RIVER_CLOUD_PLAYBACK_BACKEND_FOREIGN_ACTIVE) {
...
static void river_cloud_xiaozhi_downlink_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (backend_state != RIVER_CLOUD_PLAYBACK_BACKEND_OWNED_ACTIVE ||
            ...
            start_frames = start_gate.start_frames;
        ...
        if (river_playback_service_write((const uint8_t *)g_river_cloud.xiaozhi_downlink_stereo,
            ...
            RIVER_LOGW("xiaozhi playback rebuffer requested: cause=%s queued=%lu low=%u start=%u policy=%s cautious=%s target_ms=%u total=%lu streak=%lu recovery=%s",
                       river_cloud_playback_rebuffer_cause_name(
                           river_cloud_xiaozhi_playback_rebuffer_cause()),
                       (unsigned long)queued_frames,
                       (unsigned int)low_water_frames,
                       ...
                       (unsigned long)g_river_cloud.xiaozhi_playback_rebuffer_streak,
                       recover_path);
            if (river_cloud_xiaozhi_request_playback_rebuffer_recovery(recover_reason,
                                                                       recover_cause,
                                                                       supply_kind,
                                                                       &recover_path) !=
                RIVER_OK) {
                RIVER_LOGW("xiaozhi playback recover fallback to fresh start: cause=%s recovery=%s",
