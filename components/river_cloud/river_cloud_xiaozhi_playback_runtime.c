...
typedef enum {
    RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_DETACHED = 0,
    RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_OWNED_ACTIVE,
    RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_FOREIGN_ACTIVE,
...
static const char *river_cloud_xiaozhi_playback_backend_state_name(
    river_cloud_xiaozhi_playback_backend_state_t state)
{
    switch (state) {
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_DETACHED:
        return "detached";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_OWNED_ACTIVE:
        return "owned_active";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_FOREIGN_ACTIVE:
        return "foreign_active";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_RESTART_PENDING:
...
static river_cloud_xiaozhi_playback_backend_state_t
river_cloud_xiaozhi_playback_backend_state(void)
{
    ...
    if (service_state == RIVER_PLAYBACK_RESTART_PENDING) {
        return RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_RESTART_PENDING;
    }
    if (!river_playback_service_state_active(service_state)) {
        return RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_DETACHED;
    }
    if (g_river_cloud.xiaozhi_playback_active) {
        return RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_OWNED_ACTIVE;
    }
    return RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_FOREIGN_ACTIVE;
}

static bool river_cloud_xiaozhi_playback_backend_needs_start(
    river_cloud_xiaozhi_playback_backend_state_t state)
{
    return state == RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_DETACHED ||
           state == RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_RESTART_PENDING;
}

static bool river_cloud_xiaozhi_playback_backend_owned(
    river_cloud_xiaozhi_playback_backend_state_t state)
{
    return state == RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_OWNED_ACTIVE ||
           state == RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_RESTART_PENDING;
}
...
{
    switch (phase) {
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_IDLE:
        return "idle";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_PREFETCHING:
        return "prefetching";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_PLAYING:
        return "playing";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_REBUFFERING:
        return "rebuffering";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_DRAINING:
        return "draining";
    default:
        return "unknown";
    }
...
{
    switch (cause) {
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_REBUFFER_CAUSE_NONE:
        return "none";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_REBUFFER_CAUSE_UPSTREAM_STARVED:
        return "upstream_starved";
    case RIVER_CLOUD_XIAOZHI_PLAYBACK_REBUFFER_CAUSE_WRITE_FAILED:
        return "write_failed";
    default:
        return "unknown";
    }
...
static river_cloud_xiaozhi_playback_phase_t river_cloud_xiaozhi_compute_playback_phase(void)
{
    if (g_river_cloud.xiaozhi_tts_stop_pending) {
        return RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_DRAINING;
    }
    if (g_river_cloud.xiaozhi_playback_rebuffer_pending) {
        return RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_REBUFFERING;
    }
    if (g_river_cloud.xiaozhi_playback_active) {
        return RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_PLAYING;
    }
    if (river_cloud_xiaozhi_playback_queued_frames() != 0U) {
        return RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_PREFETCHING;
    }
...
static bool river_cloud_xiaozhi_refresh_playback_phase(const char *reason)
{
    ...
    RIVER_LOGI("xiaozhi playback phase: %s -> %s reason=%s queued=%lu active=%s backend=%s rebuffer=%s cause=%s stop=%s",
               river_cloud_xiaozhi_playback_phase_name(old_phase),
               river_cloud_xiaozhi_playback_phase_name(new_phase),
               ...
               river_cloud_xiaozhi_playback_backend_state_name(
...
{
    river_cloud_xiaozhi_playback_phase_t phase = river_cloud_xiaozhi_playback_phase();

    return phase == RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_PLAYING ||
           phase == RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_DRAINING;
}

bool river_cloud_xiaozhi_playback_lane_engaged(void)
{
    return river_cloud_xiaozhi_playback_phase() != RIVER_CLOUD_XIAOZHI_PLAYBACK_PHASE_IDLE;
}
...
static bool river_cloud_xiaozhi_playback_rebuffer_history_active(void)
{
...
void river_cloud_xiaozhi_dump_playback_status(uint64_t now_ms)
{
    ...
    RIVER_LOGI("xiaozhi duplex_ready=%s reason=%s aec=%s ref_state=%s ref_activity=%s ref_peak=%u ref_ratio_q15=%u ref_queue=%lu/%lu ref_age_ms=%lu playback_active=%s backend=%s duplex_seen=%s",
               ...
               g_river_cloud.xiaozhi_playback_active ? "yes" : "no",
               river_cloud_xiaozhi_playback_backend_state_name(backend_state),
    ...
    RIVER_LOGI("xiaozhi downlink queue=%lu/%u dropped=%lu worker=%s sample=%luHz frame=%lums start=%u target_ms=%lu meta_gap_ms=%lu backend=%s rebuffer=%s/%s rebuffer_total=%lu streak=%lu",
...
static uint32_t river_cloud_xiaozhi_downlink_start_threshold_frames(void)
{
    ...
    uint32_t threshold = g_river_cloud.xiaozhi_playback_rebuffer_pending ?
                             RIVER_CLOUD_XIAOZHI_DOWNLINK_REBUFFER_START_FRAMES :
                             RIVER_CLOUD_XIAOZHI_DOWNLINK_START_FRAMES;

    if (river_cloud_xiaozhi_playback_rebuffer_history_active()) {
        ...
        if (river_cloud_xiaozhi_rebuffer_prefetch_target_needed() && frame_ms != 0U &&
            ...
            if (prefetch_frames > threshold) {
                threshold = prefetch_frames;
            }
        }
        if (g_river_cloud.xiaozhi_playback_rebuffer_streak > 1U) {
            ...
            if (threshold >= max_start_frames) {
                extra_frames = 0U;
            } else if (extra_frames > max_start_frames - threshold) {
                extra_frames = max_start_frames - threshold;
            }
            threshold += extra_frames;
        }
    }

    if (threshold > max_start_frames) {
        threshold = max_start_frames;
    }
    return threshold;
}
...
static void river_cloud_xiaozhi_finish_playback_rebuffer(uint64_t resumed_at_ms)
{
    ...
    if (segment != NULL && segment->valid && segment->started && segment->paused_at_ms != 0U &&
        ...
        RIVER_LOGW("xiaozhi playback rebuffer resumed: cause=%s pause_ms=%lu queued=%lu total=%lu streak=%lu",
                   river_cloud_xiaozhi_playback_rebuffer_cause_name(cause),
...
static bool river_cloud_xiaozhi_maybe_rebuffer_starved(uint32_t queued_frames, uint64_t now_ms)
{
    ...
    if (!g_river_cloud.xiaozhi_playback_active || g_river_cloud.xiaozhi_tts_stop_pending ||
        ...
        backend_state != RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_OWNED_ACTIVE) {
        river_cloud_xiaozhi_clear_downlink_starvation_watch();
        return false;
    }
    ...
    river_cloud_xiaozhi_note_playback_rebuffer(
        now_ms,
        RIVER_CLOUD_XIAOZHI_PLAYBACK_REBUFFER_CAUSE_UPSTREAM_STARVED);
    RIVER_LOGW("xiaozhi playback upstream gap rebuffer: cause=%s wait_ms=%lu supply_gap_ms=%lu trigger_ms=%u queued=%lu low=%u start=%u target_ms=%u total=%lu streak=%lu",
               river_cloud_xiaozhi_playback_rebuffer_cause_name(
                   river_cloud_xiaozhi_playback_rebuffer_cause()),
                                                          ...
                                                          backend_state)
{
    ...
    if (backend_state == RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_OWNED_ACTIVE) {
...
void river_cloud_xiaozhi_playback_note_meta(const river_xiaozhi_event_t *event)
{
    ...
    RIVER_LOGI("xiaozhi playback prefetch: segment_id=%s expected_ms=%lu meta_gap_ms=%lu target_ms=%lu queued=%lu rebuffer=%s/%s rebuffer_total=%lu streak=%lu",
               g_river_cloud.xiaozhi_playback_segment_id[0] != '\0' ?
                   g_river_cloud.xiaozhi_playback_segment_id :
                   "-",
               (unsigned long)g_river_cloud.xiaozhi_playback_expected_duration_ms,
...
void river_cloud_xiaozhi_playback_check_pending_stop(void)
{
    ...
    if (!river_cloud_xiaozhi_playback_backend_owned(backend_state)) {
        if (river_cloud_xiaozhi_playback_queued_frames() != 0U) {
            river_cloud_xiaozhi_clear_playback_terminal_wait();
            return;
        }
    ...
    const char *detail_reason)
{
    ...
    bool stream_was_active = river_cloud_xiaozhi_playback_backend_owned(backend_state);
    ...
    RIVER_LOGI("xiaozhi playback abort: cause=%s clear_reason=%s stream_reason=%s interrupt=%s had_work=%s stream_active=%s backend=%s",
                                                                   ...
                                                                   size_t mono_bytes)
{
    ...
    if (backend_state == RIVER_CLOUD_XIAOZHI_PLAYBACK_BACKEND_FOREIGN_ACTIVE) {
    ...
    RIVER_LOGI("xiaozhi playback start: %luHz frame=%lums mono=%luB queued=%lu start=%u target_ms=%u mode=%s buffer=%u gain=%d/%d from_backend=%s rebuffer=%s/%s",
...
static void river_cloud_xiaozhi_downlink_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (river_playback_service_write((const uint8_t *)g_river_cloud.xiaozhi_downlink_stereo,
                                         ...
                                         mono_bytes,
                                         true) != RIVER_OK) {
            river_cloud_xiaozhi_playback_rebuffer_cause_t recover_cause =
                RIVER_CLOUD_XIAOZHI_PLAYBACK_REBUFFER_CAUSE_WRITE_FAILED;
            uint64_t supply_gap_ms = 0U;
            uint32_t low_water_frames =
            ...
            const char *recover_path = "recover";

            now_ms = (uint64_t)rtos_time_get_current_system_time_ms();
            if (river_cloud_xiaozhi_write_failed_prefers_starved_rebuffer(queued_frames,
                                                                          now_ms,
                                                                          &supply_gap_ms)) {
                recover_cause = RIVER_CLOUD_XIAOZHI_PLAYBACK_REBUFFER_CAUSE_UPSTREAM_STARVED;
            ...
            RIVER_LOGW("xiaozhi playback write failed: cause=%s mono=%luB stereo=%luB queued=%lu low=%u supply_gap_ms=%lu recovery=%s",
                       ...
                       (unsigned long)supply_gap_ms,
                       recover_path);
            RIVER_LOGW("xiaozhi playback rebuffer requested: cause=%s queued=%lu low=%u start=%u target_ms=%u total=%lu streak=%lu recovery=%s",
                       river_cloud_xiaozhi_playback_rebuffer_cause_name(
                           river_cloud_xiaozhi_playback_rebuffer_cause()),
                       (unsigned long)queued_frames,
                       (unsigned int)low_water_frames,
            ...
            if (recover_status != RIVER_OK) {
                RIVER_LOGW("xiaozhi playback recover fallback to fresh start: cause=%s status=%d",
                           river_cloud_xiaozhi_playback_rebuffer_cause_name(recover_cause),
                           (int)recover_status);
            }
            rtos_time_delay_ms(RIVER_CLOUD_XIAOZHI_DOWNLINK_POLL_MS);
            continue;
