    ...
    uint32_t wait_ms)
{
    ...
    if (request->op == RIVER_CLOUD_XIAOZHI_CTRL_PLAYBACK_MARK &&
        ...
        for (uint32_t pending_count = 0U;
             ...
             pending_count++) {
            const river_cloud_xiaozhi_control_request_t *pending =
            ...
            pending_index++;
            if (pending_index >= RIVER_CLOUD_XIAOZHI_CONTROL_QUEUE_DEPTH) {
                pending_index = 0U;
            }
        }
    }
...
void river_cloud_xiaozhi_handle_transport_event(const river_xiaozhi_event_t *event)
{
    ...
    case RIVER_XIAOZHI_EVENT_RESPONSE_START:
        ...
        river_cloud_xiaozhi_clear_accepted_response_watchdog();
...
static void river_cloud_xiaozhi_check_accepted_response_watchdog(void)
{
    river_cloud_xiaozhi_turn_semantics_state_t *state = &g_river_cloud.xiaozhi_turn_semantics;
    uint64_t now_ms;

    if (g_river_cloud.xiaozhi_session_window_truth.accepted_response_deadline_ms == 0U) {
        return;
    }
    if (river_cloud_xiaozhi_response_start_pending()) {
        return;
    }

    now_ms = (uint64_t)rtos_time_get_current_system_time_ms();
    if (now_ms <
        g_river_cloud.xiaozhi_session_window_truth.accepted_response_deadline_ms) {
        return;
    }

    river_cloud_xiaozhi_clear_accepted_response_watchdog();
    RIVER_LOGW("xiaozhi accepted response watchdog timeout: turn_id=%s accept_reason=%s input_state=%s output_state=%s session=%s window=%s sid=%s",
    ...
    if (river_xiaozhi_session_open()) {
        (void)river_cloud_xiaozhi_request_abort("accepted_response_watchdog");
    }
    ...
    river_cloud_xiaozhi_window_touch(RIVER_CLOUD_XIAOZHI_WAKE_WINDOW_FOLLOWUP_MS,
                                     "accepted_response_watchdog");
    river_cloud_request_state_sync("accepted_response_watchdog");
}
...
static bool river_cloud_xiaozhi_server_endpoint_accepts_turns(void)
{
    return river_xiaozhi_discovery_server_endpoint_enabled();
}
    ...
    const char *detail_reason)
{
    ...
    if (river_cloud_xiaozhi_server_endpoint_accepts_turns()) {
        river_cloud_xiaozhi_prepare_server_accept_wait(detail_reason);
    } else {
    ...
    const char *detail_reason)
{
    ...
    if (river_cloud_xiaozhi_server_endpoint_accepts_turns()) {
        RIVER_LOGI("xiaozhi local stop waits for server endpoint accept: cause=%s trigger=%s",
                   cause_name,
                   reason);
...
static void river_cloud_xiaozhi_arm_accepted_response_watchdog(
    ...
    const char *trigger)
{
    ...
    g_river_cloud.xiaozhi_session_window_truth.accepted_response_deadline_ms =
        now_ms + (uint64_t)RIVER_CLOUD_XIAOZHI_ACCEPTED_RESPONSE_WATCHDOG_MS;
    RIVER_LOGI("xiaozhi accepted response watchdog armed: timeout_ms=%u trigger=%s turn_id=%s accept_reason=%s sid=%s",
               (unsigned int)RIVER_CLOUD_XIAOZHI_ACCEPTED_RESPONSE_WATCHDOG_MS,
               trigger != NULL ? trigger : "-",
               (state != NULL && state->turn_id[0] != '\0') ? state->turn_id : "-",
               (state != NULL && state->accept_reason[0] != '\0') ?
                   state->accept_reason :
                   "-",
               river_cloud_xiaozhi_current_sid() != NULL ?
                   river_cloud_xiaozhi_current_sid() :
...
static bool river_cloud_xiaozhi_response_start_pending(void)
{
    return g_river_cloud.xiaozhi_playback_lineage_truth.stage ==
               RIVER_CLOUD_XIAOZHI_PLAYBACK_LINEAGE_RESPONSE_STARTED &&
           g_river_cloud.xiaozhi_playback_lineage_truth.meta_context.response_id[0] ==
               '\0';
}

static void river_cloud_xiaozhi_note_empty_turn_returned_active(
    const river_cloud_xiaozhi_turn_semantics_state_t *state,
    const char *trigger)
{
    uint64_t now_ms;
    uint64_t window_left_ms = 0U;
    ...
    now_ms = (uint64_t)rtos_time_get_current_system_time_ms();
    if (g_river_cloud.xiaozhi_session_window_truth.window_deadline_ms > now_ms) {
        window_left_ms =
            g_river_cloud.xiaozhi_session_window_truth.window_deadline_ms - now_ms;
    }
    if (window_left_ms == 0U) {
        window_left_ms = (uint64_t)RIVER_CLOUD_XIAOZHI_WAKE_WINDOW_FOLLOWUP_MS;
    }

    g_river_cloud.xiaozhi_session_window_truth.empty_turn_recover_deadline_ms =
        now_ms + window_left_ms;
    river_cloud_xiaozhi_clear_accepted_response_watchdog();
    river_cloud_xiaozhi_window_touch(RIVER_CLOUD_XIAOZHI_WAKE_WINDOW_FOLLOWUP_MS,
                                     "empty_turn_returned_active");
    RIVER_LOGI("xiaozhi empty turn returned active: trigger=%s turn_id=%s accept_reason=%s input_state=%s output_state=%s window_left_ms=%lu sid=%s",
               trigger != NULL ? trigger : "-",
               state->turn_id[0] != '\0' ? state->turn_id : "-",
               state->accept_reason[0] != '\0' ? state->accept_reason : "-",
               state->input_state[0] != '\0' ? state->input_state : "-",
               state->output_state[0] != '\0' ? state->output_state : "-",
               (unsigned long)window_left_ms,
               river_cloud_xiaozhi_current_sid() != NULL ?
                   river_cloud_xiaozhi_current_sid() :
                   "-");
    river_xiaozhi_clear_session_update_cache();
    river_cloud_xiaozhi_clear_preview_state();
    river_cloud_xiaozhi_clear_turn_semantics_state();
    river_cloud_request_state_sync("empty_turn_returned_active");
}
    ...
    const char *trigger)
{
    ...
    if (river_cloud_xiaozhi_playback_terminal_closed()) {
        river_cloud_xiaozhi_clear_empty_turn_returned_active_state();
        river_cloud_xiaozhi_clear_accepted_response_watchdog();
        RIVER_LOGI("xiaozhi response output idle already terminal: trigger=%s session_state=%s output_state=%s",
                   trigger != NULL ? trigger : "-",
                   session_state,
                   (state != NULL && state->output_state[0] != '\0') ? state->output_state : "-");
        return;
    ...
    const char *trigger)
{
    ...
    RIVER_LOGI("xiaozhi server committed input; closing local round without audio.in.commit: trigger=%s accept_reason=%s input_state=%s output_state=%s stream=%s listening=%s",
               ...
               g_river_cloud.xiaozhi_session_window_truth.listening ? "yes" : "no");
    river_cloud_xiaozhi_clear_empty_turn_returned_active_state();
    river_cloud_xiaozhi_arm_accepted_response_watchdog(state, trigger);
    river_cloud_xiaozhi_close_local_round_for_cause(
        RIVER_CLOUD_XIAOZHI_ROUND_CLOSE_SERVER_RESPONSE,
...
void river_cloud_xiaozhi_round_finish(const char *reason)
{
    ...
    if (uplink_count > 0U) {
        ...
        send_interval_p95 = river_cloud_xiaozhi_round_percentile(
            ...
            uplink_count,
            95U);
        send_interval_max = river_cloud_xiaozhi_round_percentile(
            g_river_cloud.xiaozhi_asr_round_truth.uplink_send_interval_samples,
            uplink_count,
            100U);
        ...
        capture_age_p95 = river_cloud_xiaozhi_round_percentile(
            ...
            uplink_count,
            95U);
        capture_age_max = river_cloud_xiaozhi_round_percentile(
            g_river_cloud.xiaozhi_asr_round_truth.uplink_capture_age_samples,
            uplink_count,
            100U);
        backlog_p95 = river_cloud_xiaozhi_round_percentile(
            ...
            uplink_count,
            95U);
        backlog_max = river_cloud_xiaozhi_round_percentile(
            g_river_cloud.xiaozhi_asr_round_truth.uplink_backlog_samples,
            uplink_count,
            100U);
        ...
        send_duration_p95 = river_cloud_xiaozhi_round_percentile(
            ...
            uplink_count,
            95U);
        send_duration_max = river_cloud_xiaozhi_round_percentile(
            g_river_cloud.xiaozhi_asr_round_truth.uplink_send_duration_samples,
            uplink_count,
            100U);
    }
    ...
    RIVER_LOGI("xiaozhi asr round finish: id=%lu sid=%s reason=%s duration_ms=%lu audio_ms=%lu realtime_gap_ms=%lu pace_pct=%lu first_packet_delay_ms=%lu pre_roll_frames=%lu packets=%lu frame_bytes=%u burst_max=%lu preview_warmup[target_ms=%u done_ms=%lu bypass=%lu] busy=%lu fail=%lu stale_drop=%lu ring_drop=%lu dropped_frames=%lu partial=%lu final=%lu seen[partial=%s final=%s] uplink_ms[send_interval_p50=%lu send_interval_p95=%lu send_interval_max=%lu capture_age_p50=%lu capture_age_p95=%lu capture_age_max=%lu backlog_p95=%lu backlog_max=%lu send_duration_p50=%lu send_duration_p95=%lu send_duration_max=%lu]",
               (unsigned long)g_river_cloud.xiaozhi_asr_round_truth.id,
               river_cloud_xiaozhi_current_sid() != NULL ?
                   river_cloud_xiaozhi_current_sid() :
                   "-",
               ...
               (unsigned long)g_river_cloud.xiaozhi_asr_round_truth.pre_roll_frames,
               (unsigned long)g_river_cloud.xiaozhi_asr_round_truth.packets_sent,
               (unsigned int)RIVER_CLOUD_XIAOZHI_UPLINK_PCM_FRAME_MAX,
               (unsigned long)g_river_cloud.xiaozhi_asr_round_truth.burst_max,
               ...
               (unsigned long)(stale_delta + ring_drop_delta),
               (unsigned long)g_river_cloud.xiaozhi_asr_round_truth.partial_count,
               (unsigned long)g_river_cloud.xiaozhi_asr_round_truth.final_count,
               g_river_cloud.xiaozhi_asr_round_truth.partial_seen ? "yes" : "no",
               g_river_cloud.xiaozhi_asr_round_truth.final_seen ? "yes" : "no",
